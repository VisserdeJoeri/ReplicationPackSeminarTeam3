...
function ans = get_mort_stats(output_fn, f2_perc_list, type_list, year_list, sex_list, age_list, target_cut_list, ...
  ...
  %% 3-bin function call.
  %% Determine which data to use depending on the number of education bins
  if nargin == 13
    ...
    elseif strcmp(src_data, 'ybin_ed3')
      ...
      table_mort_means = readtable(output_path + '/matlab_inputs/mort_means_3.csv');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% COPY LOCAL PATH APPENDED_RANK_MORT.CSV & MORT_MEANS.CSV
    %% LET TYPE BE 'MAIN' IN CALC_ALL_MORT_BOUNDS_PARALLEL.M??
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
